...
public struct SerpentSerializer<T>: DataResponseSerializerProtocol {
    
    public typealias SerializedObject = T
    
    let parsingHandler: (( _ data: Any? ) -> SerializedObject?)?
    ...
    public func serialize(request: URLRequest?, response: HTTPURLResponse?, data: Data?, error: Error?) throws -> T {
        ...
        let json = JSONResponseSerializer.init(options: .allowFragments, emptyResponseCodes: [], emptyRequestMethods: [])
        let result = try json.serialize(request: request, response: response, data: data, error: error)
        
        if let parsedObject: T = parsingHandler?( result ) {
            NotificationCenter.default.post(name: Notification.Name(rawValue: Parser.APICallSucceededNotification), object: nil)
            return parsedObject
        } else {
            throw NSError(domain: "Serializable.Parser", code: 2048, userInfo: [ NSLocalizedDescriptionKey : "Parsing block failed!", "JSONResponse" : result])
        }
    }
    ...
    public func responseSerializable<T:Decodable>(_ completionHandler: @escaping (DataResponse<T>) -> Void,
        ...
        let serializer: SerpentSerializer<T> = SerpentSerializer.init { (data) -> T? in
            if let sourceDictionary = data as? NSDictionary {
                ...
                return T(dictionary: unwrappedDictionary)
